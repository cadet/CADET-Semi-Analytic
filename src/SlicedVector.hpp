...
#ifndef CASEMA_SLICEDVECTOR_HPP_
#define CASEMA_SLICEDVECTOR_HPP_
...
public:
	...
	{
		casema_assert(!empty());
		casema_assert(sliceSize(slices() - 1) > 0);
	 ...
	 */
	inline size_type slices() const CASEMA_NOEXCEPT { return _index.size() - 1; }
	 ...
	 */
	inline size_type size() const CASEMA_NOEXCEPT { return _values.size(); }
	 ...
	 */
	inline T* data() CASEMA_NOEXCEPT
	{
		return _values.data();
	}

	inline T const* data() const CASEMA_NOEXCEPT
	{
		return _values.data();
	}
	 ...
	 */
	inline size_type* indices() CASEMA_NOEXCEPT
	{
		return _index.data();
	}

	inline size_type const* indices() const CASEMA_NOEXCEPT
	{
		return _index.data();
	}
...
#endif  // CASEMA_SLICEDVECTOR_HPP_
