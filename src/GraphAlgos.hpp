...
#ifndef CASEMA_GRAPHALGOS_HPP_
#define CASEMA_GRAPHALGOS_HPP_
...
namespace graph
{
	 ...
	 */
	util::SlicedVector<int> adjacencyListFromConnectionList(int const* conList, int nUnits, int nCon);

	/**
	 * @brief      Converts connection list to reverse adjacency list
	 *
	 * @param      conList  Connection list (6 items per line)
	 * @param[in]  nUnits   Number of unit operations
	 * @param[in]  nCon     Number of connections in @p conList
	 *
	 * @return     Reverse adjacency list for each unit operation
	 */
	util::SlicedVector<int> reverseAdjacencyListFromConnectionList(int const* conList, int nUnits, int nCon);
	 ...
	 */
	bool topologicalSort(const util::SlicedVector<int>& adjList, std::vector<int>& topoOrder);

} // namespace graph

} // namespace casema

#endif  // CASEMA_GRAPHALGOS_HPP_
