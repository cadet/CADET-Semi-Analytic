...
namespace graph
{
	...
	util::SlicedVector<int> reverseAdjacencyListFromConnectionList(int const* conList, int nUnits, int nCon)
	{
		...
		for (int i = 0; i < nUnits; ++i)
		{
			...
			for (int j = 0; j < nCon; ++j)
			{
				const int from = conList[6*j+1];
				if (from != i)
					continue;

				const int to = conList[6*j];
	...
	namespace detail
	{
		...
		bool topologicalSortHelper(const util::SlicedVector<int>& adjList, int u, std::vector<char>& colors, std::vector<int>& topoOrder)
		{
			// Set color to gray (1)
			colors[u] = 1;
