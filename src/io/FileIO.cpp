...
class FileReader : public casema::io::IFileReader
{
public:
	FileReader() { }
	virtual ~FileReader() CASEMA_NOEXCEPT { }
	...
	{
		return _io.template scalar<uint64_t>(paramName);
	}
	...
	{
		return _io.template vector<uint64_t>(paramName);
	}
...
class FileWriter : public casema::io::IFileWriter
{
public:
	FileWriter() { }
	virtual ~FileWriter() CASEMA_NOEXCEPT { }
...
public:
	...
	virtual uint64_t getUint64(const std::string& paramName)
	{
		...
		if (p.is_boolean())
			...
			return p.template get<uint64_t>();
	}
	...
	virtual std::vector<uint64_t> getUint64Array(const std::string& paramName)
	{
		...
		if (p.is_array())
		{
			...
			return p.template get<std::vector<uint64_t>>();
		}
		else
		{
			if (p.is_boolean())
				return std::vector<uint64_t>(1, p.template get<bool>());

			return std::vector<uint64_t>(1, p.template get<int>());
		}
	}
	...
	{
		return base_t::_opened.top()->at(elementName).is_array() && (base_t::_opened.top()->at(elementName).size() > 1);
	}
 ...
 */
class JSONMemoryReaderWriterImpl : public casema::io::IMemoryIO
{
public:
	...
	virtual ~JSONMemoryReaderWriterImpl() CASEMA_NOEXCEPT
	{
		delete _root;
	}
...
public:
	...
	virtual ~JSONFileIOProxy() CASEMA_NOEXCEPT
	{
		delete _root;
	}
...
typedef JSONBaseIOWrapper<JSONFileIOProxy<casema::io::IFileReader>> JSONFileReader;
typedef JSONBaseIOWrapper<JSONFileIOProxy<casema::io::IFileWriter>> JSONFileWriter;
...
{
	if (casema::util::caseInsensitiveEquals(fileExt, "xml"))
	{
		return new FileReaderImpl<casema::io::XMLReader>();
	}
	else if (casema::util::caseInsensitiveEquals(fileExt, "json"))
	{
		return new JSONFileReader();
	}
#ifdef ENABLE_HDF5
	else if (casema::util::caseInsensitiveEquals(fileExt, "h5"))
	{
		return new FileReaderImpl<casema::io::HDF5Reader>();
	}
#endif
...
IFileWriter* createWriter(const std::string& fileExt)
{
	if (casema::util::caseInsensitiveEquals(fileExt, "xml"))
	{
		return new FileWriterImpl<casema::io::XMLWriter>();
	}
	else if (casema::util::caseInsensitiveEquals(fileExt, "json"))
	{
	...
	{
		return new FileWriterImpl<casema::io::HDF5Writer>();
	}
#endif
